...
def get_segment_type(snake: list[tuple[int, int]], i: int, last_action: Action) -> str:
    """Returns the image key for the i-th segment of the snake."""
    if i == 0:  # Head
        if last_action is not None:
            if last_action == Action.UP: return "head_up"
            if last_action == Action.DOWN: return "head_down"
            if last_action == Action.LEFT: return "head_left"
            if last_action == Action.RIGHT: return "head_right"
        # fallback to previous logic if last_action is not provided
        if len(snake) > 1:
            dr = snake[0][0] - snake[1][0]
            dc = snake[0][1] - snake[1][1]
            if dr == -1: return "head_up"
            if dr == 1: return "head_down"
            if dc == -1: return "head_left"
            if dc == 1: return "head_right"
        return "head_right"
    elif i == len(snake) - 1:  # Tail
        if len(snake) > 1:
            dr = snake[-2][0] - snake[-1][0]
            dc = snake[-2][1] - snake[-1][1]
            if dr == -1: return "tail_down"
            if dr == 1: return "tail_up"
            if dc == -1: return "tail_right"
            if dc == 1: return "tail_left"
        return "tail_right"
    else:  # Body
        prev = snake[i - 1]
        curr = snake[i]
        ...
        # Check corners
        if (prev[0] < curr[0] and next[1] < curr[1]) or (next[0] < curr[0] and prev[1] < curr[1]):
            return "body_topleft"
        if (prev[0] < curr[0] and next[1] > curr[1]) or (next[0] < curr[0] and prev[1] > curr[1]):
...
def draw_game(screen, snake_engine):
    ...
    for row in range(GRID_SIZE):
        for col in range(GRID_SIZE):
            if row == 0 or row == GRID_SIZE - 1 or col == 0 or col == GRID_SIZE - 1:
                ...
                screen.blit(images["background"], (col * CELL_SIZE, row * CELL_SIZE, CELL_SIZE, CELL_SIZE))

    # Draw apples
    for apple in snake_engine.green_apples:
        screen.blit(images["green_apple"], (apple[1]*CELL_SIZE, apple[0]*CELL_SIZE))
    ...
    for i, segment in enumerate(snake_engine.snake):
        img_key = get_segment_type(snake_engine.snake, i, snake_engine.last_action)
        screen.blit(images[img_key], (segment[1]*CELL_SIZE, segment[0]*CELL_SIZE))
